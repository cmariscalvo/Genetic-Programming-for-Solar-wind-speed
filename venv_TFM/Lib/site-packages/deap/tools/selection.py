...
def selTournament(individuals, k, tournsize, fit_attr="fitness"):
    ...
    for i in range(k):
        ...
        for aspirant in aspirants:           
            #chosen.append(max(aspirants, key=sum(attrgetter(fit_attr))))
            chosen.append(max(aspirants, key=lambda aspirant: aspirant.fitness.computeUnivalueFitness()))
    return chosen

def selTournamentSPEAII(individuals, k, tournsize, fits, fit_attr="fitness"): 
    chosen = []
    for i in range(k):
        aspirants = selRandom(individuals, tournsize)
        indices = []
        for ind in aspirants: 
            indices.append(individuals.index(ind))
        selectedFits = []
        for index in indices: 
            selectedFits.append(fits[index])
            #chosen.append(max(aspirants, key=sum(attrgetter(fit_attr))))
        finalindex = fits.index(max(selectedFits))
        chosen.append(individuals[finalindex])
    return chosen
