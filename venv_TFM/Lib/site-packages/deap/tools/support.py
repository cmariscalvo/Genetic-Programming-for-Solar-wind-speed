    ...
    def record(self, **infos):
        ...
        for key, value in list(infos.items()):
            if isinstance(value, dict):
                ...
                self.chapters[key] = chapter_infos
#                 print(f'chapter {key}', self.chapters[key])
#                 input()
                del infos[key]
        self.append(infos)
        
#         print(infos, infos.type)
#         input()
    ...
    def __txt__(self, startindex):
        ...
        #Creating dictionary with generation statistics
        dictionary = dict(self.chapters)
        #print('this is dictionary', dictionary)
        #print('dictionary', dictionary)
        import copy
        #Si se pone deepcopy, funciona. Si se pone solo copy, da error de que el dictionary cambia de tamaño
        #en la iteracion. 
        dict_copy = copy.deepcopy(dictionary)
       
        keys = dict_copy.keys()
        #Workflow is the following: 1, loop through keys. 2, loop through subkeys, assign 
        # value of subkey to a new subkey named key + subkey and finally loop again at subkeys level
        # and delete old subkeys (changing subkeys names)
        final_dict = {}
        createFinalDict = False
        for key in keys:
            subkeys = dict_copy[key].keys()
            for subkey in subkeys:
                dictionary[key][key + '_' + subkey] = [dictionary[key][subkey]]
            for subkey in subkeys: 
                del(dictionary[key][subkey])
            if createFinalDict == False: 
                final_dict = dictionary[key]
                createFinalDict = True
            else: 
                final_dict.update(dictionary[key])
        createFinalDict = False
       
        if 'DataFrameRegister' not in globals():
            global DataFrameRegister
            print('Creating pandas dataframe')
            #global DataFrameRegister
            DataFrameRegister = pandas.DataFrame(final_dict)
        else: 
            newRow = []
            for i in final_dict.values(): 
                newRow.append(i[0])
            #print(f'This is the new row: {newRow}')
            DataFrameRegister.loc[len(DataFrameRegister.index)] = newRow
            #input()
            
        final_dict = {}
        createFinalDict = False
        return(DataFrameRegister)
        
        
            #x = list(map(lambda x: [x], dictionary[key][0].values()))
            #dictionary[key][0].values() = list(x)
