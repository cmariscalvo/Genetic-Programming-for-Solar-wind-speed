...
def selNSGA2(individuals, k, nd='standard'):
    ...
    """
    if nd == 'standard':
        pareto_fronts, nonDominatedMSE, nonDominatedSize= sortNondominated(individuals, k)
    elif nd == 'log':
        pareto_fronts, nonDominatedMSE, nonDominatedSize = sortLogNondominated(individuals, k)
    else:
...
#     assignCrowdingDist(new_pareto)
    ...
    return chosen, nonDominatedMSE, nonDominatedSize, pareto_fronts
...
def sortNondominated(individuals, k, first_front_only=False):
    ...
    for fit in current_front:
        ...
        nonDominatedFitness.append(map_fit_ind[fit][0].fitness.values)
    pareto_sorted = len(fronts[-1])
    getMSE = lambda x: x[0]
    getSize = lambda x: x[1]
    nonDominatedMSE = [getMSE(x) for x in nonDominatedFitness]
    nonDominatedSize = [getSize(x) for x in nonDominatedFitness]
    ...
    return fronts, nonDominatedMSE, nonDominatedSize
...
def assignCrowdingDist(individuals):
    ...
    for i in range(nobj):
        ...
        for prev, cur, next in zip(crowd[:-2], crowd[1:-1], crowd[2:]):
            distances[cur[1]] += abs(next[0][i] - prev[0][i]) / abs(norm)
        distances[0] += abs(crowd[1][0][i]-crowd[0][0][i]) #mine
        distances[-1] += abs(crowd[-1][0][i]-crowd[-1][0][i])#mine
...
def selTournamentDCDSPEAII(individuals, k, tournsize=2):
    """Tournament selection based on dominance (D) between two individuals, if
    the two individuals do not interdominate the selection is made
    based on crowding distance (CD). The *individuals* sequence length has to
    be a multiple of 4. Starting from the beginning of the selected
    ...
    for i in range(k):
        ...
        chosen.append(tourn(aspirants[0], aspirants[1]))
    return chosen
...
#             for j in range(i + 1, N):
    ...
    individuals = [individuals[i] for i in chosen_indices]
    fits = [fits[i] for i in chosen_indices]
    return individuals, fits
