...
import random, pandas, numpy, time, multiprocessing
...
def eaSimpleOr(population, toolbox, cxpb, mutpb, ngen, stats=None,
             halloffame=None, verbose=__debug__, multi = True):
    """This algorithm reproduce the simplest evolutionary algorithm as
    presented in chapter 7 of [Back2000]_.
    ...
    if multi == True: 
        print(f'Entering multiprocessing with {multiprocessing.cpu_count()} cores')
        ...
        #invalid_ind_multi = cuda.to_device(invalid_ind)
#         fitnesses = toolbox.map(toolbox.evaluate, invalid_ind_multi)
#         del invalid_ind_multi
    else: 
        print('Entering without multiprocessing activated')
        fitnesses = toolbox.map(toolbox.evaluate, invalid_ind)
    ...
    for gen in tqdm(range(1, ngen + 1)):
        ...
        if multi == True: 
            print(f'Entering {gen}th generation')
#             invalid_ind_multi = cuda.to_device(invalid_ind)
#             fitnesses = toolbox.map(toolbox.evaluate, invalid_ind_multi)
#             del invalid_ind_multi
            with tqdm(total=len(invalid_ind)) as progress:
                with ProcessPoolExecutor(4) as executor:
                    futures = []
                    for ind in invalid_ind: 
                        future = executor.submit(toolbox.evaluate, ind)
...
def eaSimpleOrNSGAII(population, toolbox, cxpb, mutpb, ngen, stats=None,
             halloffame=None, verbose=__debug__, plot= True, multi = True):
...
#                     fitnesses = [future.result() for future in futures]
    ...
    print(f'Algorithm execution took {end - start} s')
    return population, logbook, halloffame, pareto_fronts
